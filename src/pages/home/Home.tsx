...
export default class Home extends Component {
  ...
  setTimer = () => {
    ...
    let showPoints = setTimeout(() => {
      this.setState({ modal: false });
    }, 63000);
    globalTimeouts.push(showPoints);
  };
  ...
  changeCard = () => {
    ...
    if (validRange.length > 1 && this.state.time > 0) {
      ...
      this.setState({
        modalText: `👏🏽 כל הכבוד! סיימתם ${cards.length} כרטיסים`,
        modal: true,
        finishCards: true,
      });
      setTimeout(() => {
    ...
    return (
      <div className="home-container">
        {!startGame || endGame ? (
          <>
            {!endGame && (
              <h2 className="welcome font-effect-outline">
                !ברוכים הבאים למשחק טאבו
              </h2>
            )}
        ...
        <div className="table-con">
          <MyTable group1Points={group1Points} group2Points={group2Points} />
          <Button
        ...
        >
          <div className="modal-con font-effect-outline">
            {finishCards ? modalText : <h1>{modalText}</h1>}
          </div>
        </Modal>
      </div>
    );
